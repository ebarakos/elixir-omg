...
defmodule OMG.State.Transaction.Recovered do
  ...
  defp recover_from_struct(%Transaction.Signed{raw_tx: raw_tx, sigs: sigs} = signed_tx) do
    ...
    with {:ok, reversed_spenders} <- get_reversed_spenders(hash_no_spenders, sigs),
         do:
           {:ok,
            %__MODULE__{
  ...
  defp get_reversed_spenders(hash_no_spenders, sigs) do
    sigs
    |> Enum.filter(fn sig -> sig != @empty_signature end)
    |> Enum.reduce_while({:ok, []}, fn sig, acc -> get_spender(hash_no_spenders, sig, acc) end)
  end

  defp get_spender(hash_no_spenders, sig, {:ok, spenders}) do
    Crypto.recover_address(hash_no_spenders, sig)
    |> case do
